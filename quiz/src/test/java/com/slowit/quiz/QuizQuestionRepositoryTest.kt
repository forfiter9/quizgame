...
class QuizQuestionRepositoryTest {
    ...
    @Test
    fun `check if api service is making request when repository get quiz called`() =
        runTest {
    ...
    @Test
    fun `check if repository get questions is correctly map to domain model`() =
        runTest {
            coEvery { mockQuizApiService.getQuizQuestions() } returns MOCKK_API_SERVICE_QUIZ_QUESTION_RESPONSE
    ...
    companion object {
        ...
        private val MOCKK_DOMAIN_QUIZ_QUESTION =
            listOf(
                QuizQuestion(
                    question = "test1",
                    image = "test1",
